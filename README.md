...
- native elementwise `Ops`, `Summary`, `Math`/`Math2`, and matrix
  reduction kernels; matrix products (`%*%`, `crossprod()`,
  `tcrossprod()`) call BLAS `dgemm` for finite double operands and
  managed native loops otherwise;
- typed tensors: raw fmalloc payloads in a foreign element encoding
  ...
  `dgemm`;
- a builtin lossless `"alp"` compression codec (`as_fmalloc_tensor()`)
...
- explicit `destroy_fmalloc_vector()`, an in-file persistent allocation
  catalog, runtime/vector introspection helpers, and an installed C
  header with `R_RegisterCCallable()` entry points (API version 4),
  including tensor codec registration for downstream packages.
