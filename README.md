...
# Weights land in that mmap'd file still quantized (q4_k/q5_0/q6_k/q8_0): the
# import copies the raw GGUF blocks over without decoding them. From then on the
# forward pass points GGML tensors straight at those payloads, zero-copy.
...
Five packages, one stack. Each links to its reference documentation; the
sources live under
...
| [**Rpgen**](https://github.com/sounkou-bioinfo/Rfmalloc/tree/main/packages/Rpgen) | genomics I/O: a native PLINK2 `.pgen` reader in C (vendored pgenlib, GPL-3) exposing the C-callable API pgenlibr lacks, the on-disk source the genotype codecs pack from                                                      |
