...
`fmalloc_set()`/`fmalloc_fill()` and
`fmalloc_add()`/`sub()`/`mul()`/`div()` write straight through the
backing store, bypassing R’s copy-on-modify. This is essential at
fmalloc scale (copying a larger-than-RAM payload on every `x[i] <-` is
fatal) and deliberately by-reference: all bindings to the same vector
observe the change, so these are explicit functions, never a silent
`[<-`.
