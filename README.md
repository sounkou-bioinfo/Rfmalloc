...
-   Explicit base-fallback boundaries:
    -   `rowSums()`, `colSums()`, `rowMeans()`, and `colMeans()` fall
        back to base implementations (with a warning) when the input is
...
-   In persistent mode, committed vector payloads are not returned to
    fmalloc by vector finalizers. Fixed-width atomic vectors serialize
    by reference to the physical allocation in the backing file.
...
-   `x[i]` creates a fmalloc-backed subset copy for supported vector
    types;
-   `bench::mark()` reports R heap allocation in `mem_alloc`; it does
    not count bytes stored in the fmalloc mapped file.
