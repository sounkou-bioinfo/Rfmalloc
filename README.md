...
`Rfmalloc` and `Rggml` build on Linux, macOS and Windows; `Rgguf` and
`Rllm` are Unix-only for now (Rgguf’s vendored GGUF parser memory-maps
the file with no Windows path). All four are verified on ARM in CI:
