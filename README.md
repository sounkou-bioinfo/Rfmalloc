...
The bundled model is deliberately tiny and deterministic: it makes the
evaluated example traverse the real GGUF loader, borrowed weight spans,
transformer graph, KV cache, generation loop and byte decoder without a
model download. Its output is a systems check, not a language-quality
claim.
