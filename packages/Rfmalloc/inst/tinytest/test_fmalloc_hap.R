...
(function() {
    ...
    view <- .Call("rfm_hap_view_info_impl", hap)
    expect_equal(view$n_locus, L)
    expect_equal(view$n_haplotype, N)
    expect_equal(view$row_bytes, ceiling(N / 8))
    expect_equal(view$stride %% 64, 0)
    expect_equal(view$body_alignment, 0L)
    expect_equal(view$stride_alignment, 0L)
    expect_true(view$padding_zero)
