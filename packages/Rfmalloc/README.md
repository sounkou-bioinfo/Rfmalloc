...
R object and not a proxy handle or a lazy file index, `dim()`, `%*%`,
`crossprod()`, `Ops`, and subsetting just work - but the data can be
larger than RAM, compressed on disk, mutated in place, and multiplied by
a pluggable backend.
...
- native elementwise `Ops`, `Summary`, `Math`/`Math2`, and matrix
  reductions;
- matrix products (`%*%`, `crossprod()`, `tcrossprod()`) call BLAS
...
- **out-of-core** products (`fmalloc_matmul_ooc()`,
  `fmalloc_crossprod_ooc()`, `fmalloc_tcrossprod_ooc()`) that stream
  column tiles through `dgemm` with `madvise` eviction, so operands
  larger than RAM multiply with a bounded resident set;
  `%*%`/`crossprod`/`tcrossprod` auto-route to these above a RAM-based
  threshold.

Compression (typed tensors):

- `as_fmalloc_tensor()` stores a matrix under a codec - lossless `"alp"`
  for decimal-scaled doubles, `"sparse"` for mostly-zero
  (single-cell/count) data - and the compressed tensor still
  participates in the panel-streamed matrix products;
...
- `fmalloc_set()`, `fmalloc_fill()`, and
  `fmalloc_add()`/`sub()`/`mul()`/`div()` write through the backing
  store, bypassing R’s copy-on-modify - essential when the payload is
  larger than RAM or persistent.
...
- `rowSums()`/`colSums()`/`rowMeans()`/`colMeans()` fall back to base
  (with a warning) when the input is not an exact 2D matrix or
  `dims != 1L`;
...
`%*%`, `crossprod()`, and `tcrossprod()` on fmalloc matrices call BLAS
`dgemm` when both operands are finite doubles, and managed native loops
otherwise (the same split base R’s default matrix product uses). Results
are fmalloc-backed.
...
`fmalloc_tensor_materialize()` handle any rank - while the matrix
products, which decode the payload in bounded, block-aligned column
panels streamed through `dgemm` (so the full double representation is
never materialized), require exactly 2 dimensions. Codecs `f64`, `f32`,
`f16`, and `bf16` are builtin; downstream packages register more through
the C API (Rgguf adds the quantized GGUF formats).
...
`fmalloc_matmul_ooc()`, `fmalloc_crossprod_ooc()`, and
`fmalloc_tcrossprod_ooc()` stream a matrix through `dgemm` one
contiguous column tile at a time, releasing each tile’s pages with
`madvise(MADV_DONTNEED)`, so a matrix larger than RAM multiplies with a
resident set bounded by the tile size rather than the matrix size.
`crossprod(X)` = `X'X` is the covariance/Gram matrix behind PCA, ridge,
and GWAS.

`%*%`/`crossprod()`/`tcrossprod()` on an fmalloc matrix auto-route to
these when the operand reaches `getOption("Rfmalloc.ooc_threshold_gb")`
(default half of physical RAM). The example forces the threshold to 0 to
show the path on a small matrix:
...
`fmalloc_set()`/ `fmalloc_fill()` and
`fmalloc_add()`/`sub()`/`mul()`/`div()` mutate by reference *regardless
of sharing*, so all bindings observe the change - deliberately, and only
ever through these explicit functions, never a silent `[<-`.
