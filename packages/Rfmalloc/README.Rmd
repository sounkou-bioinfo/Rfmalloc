...
The examples use temporary backing files. Each snippet uses `local()` to keep
its `on.exit()` cleanup scoped. In your own code, keep the runtime handle for
as long as its arrays are live and call `cleanup_fmalloc()` when finished.
...
For persistent runtimes, list containers serialize as reference state when all
...
catalog can be listed, but it does not provide a high-level object store for
recovering vectors by name.
