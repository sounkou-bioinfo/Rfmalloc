...
- `Rggml` is a low-level **carrier package**. Its generated static library
  ...
  and the opt-in Vulkan and CUDA backends. Sibling packages consume these through
...
- Linux, macOS, and Windows are supported. `configure.win` reuses the main
  ...
  the build recipe.
- Attribution/licensing lives in `inst/COPYRIGHTS` (GGML and the retained
  ggmlR I/O shim are MIT; Rggml's own code is GPL (>= 2)).
