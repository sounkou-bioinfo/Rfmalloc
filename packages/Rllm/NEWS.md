...
- Native inference now consumes a bound `rllm_program`: the serializable AST,
...
- Added complete Qwen3.5 execution against the real Ternary Bonsai 27B
  metadata and tensor layout. Its serializable R program distinguishes 48
  gated-delta recurrent blocks from 16 gated-attention blocks, represents both
  recurrent and KV state, and binds all 851 parameters without a C++ model
  branch. Whole-batch and incremental paths match a pure-R oracle and run on
  CUDA with plain and fmalloc-backed state; all 248,320 token-zero logits from
  the real checkpoint match a portable upstream llama.cpp build bit-for-bit.
...
- Added a data-only architecture program traced from ordinary R modules and
  base pipes. It preserves several typed inputs, several named operator
  results, parameter identities, representation taps and structured shared
  loops. `rllm_execute()` runs dataflow and carried loops through explicit R
...
- `rllm_gguf_model()` now borrows every weight directly from the read-only
  ...
  retains the mapping, and GGML receives the original encoded pointers.
...
- `q5_0`, `q5_1`, `q3_k`, and `q5_k` remain accepted by quantization and the
  typed-GEMM bridge. Their Rfmalloc codec registration now lives with every
  other GGUF storage codec in Rgguf and uses Rggml's reference decoder.
- Added the **llama-architecture forward pass**: `rllm_gguf_model()` loads a
  GGUF model's hyperparameters and exposes weights in their native
  `q4_k`/`f32`/... encoding, and `rllm_forward()` assembles the GGML compute graph (RMSNorm,
  ...
  kernels without ever being decoded to double. Verified against a pure-R reference implementation of the
