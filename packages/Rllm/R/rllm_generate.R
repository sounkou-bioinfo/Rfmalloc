...
#' Generate tokens greedily with a KV cache
#'
#' The bytes-in/bytes-out generation entry point: prefills a KV cache with the
#' prompt (ids or raw bytes), then decodes greedily one token per step -
#' each step is a single-token [rllm_forward()] over the cache, not a full
#' re-forward. Stops after `n_new` tokens or at the model's EOS token.
#'
...
#' @export
rllm_generate <- function(model, prompt, n_new = 32L, cache = NULL,
                          runtime = NULL) {
    ...
    for (step in seq_len(n_new)) {
        nxt <- which.max(logits[, ncol(logits)]) - 1L
