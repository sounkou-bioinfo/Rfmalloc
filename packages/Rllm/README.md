...
array stack: it wires [Rfmalloc](../Rfmalloc) (file-backed,
memory-mapped storage), [Rgguf](../Rgguf) (GGUF model files as typed
storage views) and [Rggml](../Rggml) (vendored GGML compute with
...
parameters directly. `rllm_plan()` constructs a layer-oriented view only
...
An architecture is described with ordinary R functions. `rllm_module()`
gives a `forward` function a persistent identity, the base pipe records
dataflow, `rllm_residual()` records branches, `rllm_tap()` names
...
evictable and open to a different cache codec without changing the
graph. `rllm_generate()` prefills that cache and then performs EOS-aware
