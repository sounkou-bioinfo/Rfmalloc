...
The example runs inside `local()` so its `on.exit()` cleanup stays scoped to
the snippet while the README is rendered; in your own code, keep the runtime
handle you need and call `Rfmalloc::cleanup_fmalloc()` when finished.
