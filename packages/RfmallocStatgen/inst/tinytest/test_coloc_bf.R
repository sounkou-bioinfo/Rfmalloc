...
got <- statgen_coloc_bf(bf1, bf2, backend = "cpu", block_size = 37L)
...
## Panel boundaries affect accumulation order only, not the answer.
got_one <- statgen_coloc_bf(bf1, bf2, backend = "cpu", block_size = n)
expect_equal(got, got_one, tolerance = 1e-12)
...
expect_error(statgen_coloc_bf(bf1, bf2, block_size = 0), "block_size")
