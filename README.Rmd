...
A comprehensive stress test script is provided that demonstrates the full capabilities:
...
- **File Creation**: Large files are created using sparse allocation (fast)
- **Vector Allocation**: Memory mapping provides near-instant allocation
- **Data Access**: Random access performance similar to regular R vectors
- **Memory Efficiency**: Only allocated regions consume actual disk space
...
To validate the package can handle large-scale workloads, a comprehensive stress test is included:
...
The **demo stress test** creates a 1GB backing file and demonstrates large vector operations safely.

The **full stress test** creates a 50GB backing file and allocates multiple gigabyte-sized vectors to verify system stability and performance under extreme load.
