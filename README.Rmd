...
The examples use temporary backing files. Each snippet is executed through
`run_readme_example()` so `on.exit()` cleanup remains local to the snippet while
the README is rendered; this is not required in normal interactive use. In your
own code, keep the runtime handle you need and call `cleanup_fmalloc()` when
finished.
