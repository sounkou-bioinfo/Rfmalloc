...
Rfmalloc provides memory-mapped file allocation capabilities for R using the fmalloc library. It offers two main approaches for efficient persistent storage and memory usage:

1. **ALTREP-based memory-mapped vectors** - Custom vector types that directly map file contents to memory
2. **Custom allocators using fmalloc** - Alternative memory allocation backed by memory-mapped files
